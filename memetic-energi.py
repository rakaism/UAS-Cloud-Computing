...
# Fungsi evaluasi untuk memetic algorithm
def evaluate(solution):
    return calculate_total_energy(solution)

# Fungsi lokal search (Hill Climbing) untuk memetic algorithm
def local_search(solution):
    best_solution = solution
    best_energy = evaluate(solution)

    for _ in range(10):  # Lakukan pencarian lokal sebanyak 10 iterasi
        neighbor = mutate(solution, mutation_rate=0.1)
...
def memetic_algorithm(population_size, generations, tournament_size, crossover_rate, mutation_rate):
    ...
    for generation in range(generations):
        ...
        elite = sorted(population, key=lambda x: evaluate(x))[:elite_size]
        new_population.extend(elite)

        # Generasi baru dengan crossover, mutasi, dan lokal search
    ...
    best_solution = min(population, key=lambda x: evaluate(x))
    best_energy = evaluate(best_solution)

    return best_solution, best_energy

# Jalankan algoritma memetic
best_solution_memetic, best_energy_memetic = memetic_algorithm(
    population_size=100, generations=50, tournament_size=5, crossover_rate=0.7, mutation_rate=0.2
)

# Tampilkan hasil
if best_solution_memetic is None:
    print("Tidak ditemukan solusi yang memenuhi")
else:
    print("Kombinasi tugas dan perangkat mobile dengan energi minimum (Memetic Algorithm):")
    for job, device in best_solution_memetic.items():
        print("Tugas", job, "-> Perangkat Mobile", device)
    print("Total energi:", best_energy_memetic)
    solution_list_memetic = list(best_solution_memetic.values())
    print("Kombinasi terbaik:", solution_list_memetic)
    print("Jumlah energi terendah:", best_energy_memetic)
